...
def dgs(folder, density, doplot, resolution):
   ...
   #intervals = [x/100. for x in range(0,100,5)]

   pd = np.interp([.05,.1,.16,.25,0.3,.5,.75,.84,.9,.95],np.hstack((0,np.cumsum(yi))), np.hstack((0,xi)) )
   
   #pd = np.interp(intervals,np.hstack((0,np.cumsum(yi))), np.hstack((0,xi)) )
